...
def die(message):
    print("[!] " + message)
    exit(1)  # ← was exit() with no arg = code 0 = fake success
...
if args.seed is None and args.seedbase32 is not None:
    normalized_seed32 = str(args.seedbase32 + '=' * (-len(args.seedbase32) % 8))
    seed = base64.b16encode(base64.b32decode(normalized_seed32)).decode("utf-8")
else:
    seed = args.seed
...
def profile_has_seed(profile_num):
    """
...
if seed is not None:
    ...
    if not is_deleting:
        ...
        if profile_has_seed(profile_number):
